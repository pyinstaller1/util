...
def github():
    ...
    if str_location and str_location != "Bok일반1":
...
def a02_bok():
    ...
    if (80 < r < 100) and (88 < g < 120) and (80 < b < 100):
        ...
        for item in results:
            if item[1][0] not in ['몬', '본', '온', '{', '생', '최', '캠', '미', '마', '예', '전'] and item[1][len(item[1])-2:] not in ['협곡', '결지', '시공']  and item[1][len(item[1])-2:len(item[1])-1] not in ['7', '8']:
                print(item[1])
                print(item[1][0])
                print(item[1][len(item[1])-2:])
                print(item[1][len(item[1])-2:len(item[1])-1])
