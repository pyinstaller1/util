...
def on():
    print("더스타라이트 on   " + time.strftime("%H:%M", time.localtime()))


    '''
    for proc in psutil.process_iter():
        if "Client-Win64-Shipping.exe" in proc.name():            
            proc.kill()
            print("   더스타라이트를 닫았습니다.")
    '''
    ...
    if os.environ.get('COMPUTERNAME') == "DESKTOP-LRGAL8H":
        launcher_path = r"D:\Program Files\HiveLauncher\launcher\CrossplayLauncherHelper.exe"
        arguments = "hive:?app_id=com.com2us.thestarlight.pc.hive.kr.normal&start_point=5&envi="
        working_dir = r"D:\Program Files\HiveLauncher\launcher"

        subprocess.Popen([launcher_path, arguments], cwd=working_dir, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=False)


    else:
        launcher_path = r"C:\Program Files\HiveLauncher\launcher\CrossplayLauncherHelper.exe"
        arguments = "hive:?app_id=com.com2us.thestarlight.pc.hive.kr.normal&start_point=5&envi="
        working_dir = r"C:\Program Files\HiveLauncher\launcher"
    ...
    for i in range(10):
        if gw.getWindowsWithTitle('CrossplayLauncher'):
            print('     CrossplayLauncher')
    ...
    mouse.move(left_on+(width_on*0.8), top_on+(height_on*0.87), absolute=True, duration=0.1)   # 게임 실행
    mouse.click()
    time.sleep(5)
    ...
    for i in range(10):
        if gw.getWindowsWithTitle('더 스타라이트'):
            print('     더 스타라이트')
            win = gw.getWindowsWithTitle('더 스타라이트')[0]
            break
        time.sleep(10)
