...
def game():
    print("게임 시작")
    # win = gw.getWindowsWithTitle('LOST ARK (64-bit, DX11) v.3.10.4.1')[0]
    win = next(w for w in gw.getWindowsWithTitle('LOST ARK') if w.title.startswith("LOST ARK"))
    ...
    for j in range(170):
        ...
        if j%90==0 and j > 0:
            pyautogui.press('i')
            time.sleep(1)
            pyautogui.click(x=(left + int(width*0.95)), y=(top + int(height*0.47)), button='right')
...
def l01_start():
    ...
    global wins
    wins = [win for win in gw.getWindowsWithTitle('LOST ARK (64-bit, DX11) v.3.6.1.3') if win.title.strip()]   # LORDNINE 윈도우 목록 가져오기
