...
def a01_start(check='0'):
    print("아키텍트 a01_start   " + time.strftime("%H:%M", time.localtime()))
    ...
    win = gw.getWindowsWithTitle('Architect')[0]
    app = Application().connect(handle=win._hWnd)
...
def on():
    print("아키텍트 on   " + time.strftime("%H:%M", time.localtime()))

    for proc in psutil.process_iter():
        if "Architect.exe" in proc.name():
            proc.kill()
            print("   아키텍트를 닫았습니다.")

        if "Architect-Win64-Shipping." in proc.name():
            proc.kill()
            print("   아키텍트를 닫았습니다.")
        ...
        if "DRIMAGE LAUNCHER.exe" in proc.name():
            proc.kill()
            print("   DRIMAGE LAUNCHER.exe를 닫았습니다.")



    subprocess.Popen([r"D:\Users\Administrator\AppData\Local\DRIMAGE\Apps\Architect\Architect.exe"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    time.sleep(70)
    ...
    mouse.move(left+(width*0.5), top+(height*0.87), absolute=True, duration=0.1)   # 확인
    mouse.click()
    ...
    mouse.move(left+(width*0.87), top+(height*0.91), absolute=True, duration=0.1)   # 시작하기
    mouse.click()
    time.sleep(38)

    scr = pyautogui.screenshot(region=(left + int(width*0.46), top + int(height*0.83), int(width*0.1), int(height*0.07)))   # 구글 계정 ocr
    scr.save("scr_archi.png")
    ...
    if results and results[0][1][0] in ['확']:
        print(7)
        mouse.move(left+(width*0.5), top+(height*0.87), absolute=True, duration=0.1)   # 확인
        mouse.click()
    else:
